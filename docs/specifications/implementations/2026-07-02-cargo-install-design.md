...
**Status:** DRAFT (pending review pass)
...
- **S2:** `topgrade` is declared in `dependencies/packages.toml`
  ...
  Containerfile installs it via **`cargo binstall -y`** (not `cargo
  install --locked`), using the bootstrapped binstall. `podman exec
  dotfiles-manjaro zsh -ic 'which topgrade'` resolves after `make up`.
- **S3:** A new **`layer = 6`** is the **runtime-manual** layer. Cargo
  ...
  needs **no functional generator change**; only the spec 02 AUTO-GEN
  heading for layer 6 is special-cased to read "Layer 6 —
  runtime-manual (not build-installed)" (mirrors the existing layer-0
  "already in the base image" special case). `schema` stays at 1.
...
- **S5:** The Containerfile `toolchain` sub-layer ordering is
  ...
  `cargo install --locked` to `cargo binstall -y`, renumbered). An
...
- **S6:** `make down && make up` preserves cargo / rustup toolchain
  binaries (the named volumes persist — regression check).
...
- **S8:** Specs updated: new **`24-rust-packages-rule.md`**; spec 02
  (cargo manager rule + layer-6 note); spec 20 (a `I-CARGO` invariant if
  warranted, or a cross-ref to spec 24); spec 21 (Stage 3 rows 3-4/3-5/3-6
  + acceptance criteria for binstall/topgrade).
...
- **B2 — Build-time cargo tools via `cargo binstall -y` (chosen).**
  Requires `cargo-binstall` to be bootstrapped first (Layer 3-5), then
  Layer 3-6 reads `layer_3/cargo.txt` and runs `cargo binstall -y ${=pkgs}`
  (fast prebuilt). The Rust packages rule (spec 24) requires build-time
  `layer_3` cargo entries to have prebuilt binaries; source-only tools
  are runtime-manual (`layer = 6`).
- **C1 — Runtime-manual declaration via a new flag `runtime_only = true`
  (schema 1 → 2).** Semantically explicit; generalizes to any manager.
  Cost: schema bump + `validate` / `write_txt_files` / doc-block
  changes + tests. **Rejected in favor of C2** for minimal change.
- **C2 — Runtime-manual declaration via a new layer number `layer = 6`
  ...
  **zero functional generator change** — only a 2-line spec-02 heading
  ...
  non-build reference list. Picked for minimal change and because the
  user explicitly offered "new number or new flag."
- **C3 — Don't declare runtime-manual tools in `packages.toml` at all;
  ...
  benefit and lets the reference doc drift from the manifest.
  **Rejected.**
...
- **I2 (cargo-binstall = infra):** `cargo-binstall` is bootstrapped in
  ...
  invariant records this (spec 20), mirroring how rustup/mise are
  documented as curl-bootstrapped infra.
- **I3 (layer semantics):** `layer = 3` = build-time toolchain
  (Containerfile stage `toolchain`, installed via `cargo binstall -y`,
...
- **I4 (persistence parity):** cargo tools install under
  ...
  mountpoint); `cargo-binstall`'s own cache lives under
  `~/.cache/cargo-binstall` (or `$CARGO_HOME/bin` for the binary). The
...
- **I6 (cache reuse):** the Layer 3-6 `RUN` carries a BuildKit
  `--mount=type=cache` on `~/.cache/cargo-binstall` (binstall's download
  cache) so rebuilds reuse downloaded prebuilt archives without
  bloating image layers (mirrors the existing cargo registry/git cache
  mounts and the mise cache mount). Layer 3-5 (binstall bootstrap) also
  uses a cache mount for its download.
...
1. **Generator** — `programs/generate_deps/main.py`: special-case the
   ...
   build-installed)"), mirroring the layer-0 special case. No other
   functional change (layer-6 txt emission already works). Update
   stale comments mentioning layer numbers if any.
...
3. **Containerfile** — `container/Containerfile`: reorder/renumber the
   ...
   bootstrap)/3-6(cargo tools via `cargo binstall`). Add the binstall
   bootstrap RUN (prebuilt binary). Switch the cargo-tools RUN from
   `cargo install --locked` to `cargo binstall -y`. Add the
   `~/.cache/cargo-binstall` cache mount to 3-6 (and 3-5 as needed).
4. **Tests** — `programs/generate_deps/tests/`: update / add a test
   ...
   "runtime-manual" heading. Keep `test_cargo_manager.py` green.
...
RUN --mount=type=cache,target=/home/${USERNAME}/.cache/cargo-binstall,uid=${HOST_UID},gid=${HOST_GID} \
    zsh -c 'set -eo pipefail; source /tmp/build-home/.zshenv; \
...
- The `cargo-binstall-x86_64-unknown-linux-musl.tgz` release asset is
  the official prebuilt static binary (no glibc dependency). The exact
  asset name / install method is to be confirmed against the
  cargo-binstall release page during implementation (the upstream
  `cargo-binstall` installer script `https://cargo-binstall.app` is an
  alternative; the tarball is preferred for build reproducibility and
  to avoid the installer script's interactive prompts). **Open
  question Q1** (§7).
- `${=pkgs}` (zsh split) is preserved, matching the existing cargo/mise
  sub-layers.
- The `cargo binstall` download cache (`~/.cache/cargo-binstall`) is
  cache-mounted in both 3-5 and 3-6 so a cache-miss rebuild reuses
  downloads (I6).
- The `--no-confirm` flag makes `cargo binstall` non-interactive in the
  build (binstall otherwise prompts to confirm the binary source for
  crates that don't ship a signed provenance; the exact flag name is to
  be confirmed against `cargo binstall --help` during implementation —
  it may be `--no-confirm` or `-y` is for the prompt; **open question
  Q2**).
...
- §3 Build-time vs runtime-manual boundary:
  - **`layer = 3`** — build-installed, via `cargo binstall -y`
    (prebuilt only). Use only for tools the container must boot with
    (e.g. `topgrade`). Keep this list small (build cost + image size).
...
- §4 `cargo-binstall` itself is **infrastructure** (Containerfile
  bootstrap, not in `packages.toml`), like `rustup` / `mise`. Recorded
  as `I-CARGO1` in spec 20.
...
- `02-installed-programs.md`: regenerate AUTO-GEN (via `make gen-deps`);
  add a prose note to the `cargo` manager rule: "Build-time cargo tools
  (`layer = 3`) are installed via `cargo binstall -y` (prebuilt only);
  `layer = 6` is the runtime-manual reference layer (not build-installed).
  See [`24-rust-packages-rule.md`](24-rust-packages-rule.md)."
- `20-container-rules.md`: add `I-CARGO1` (`cargo-binstall` is infra,
  curl-bootstrapped, not in `packages.toml`, like rustup/mise); add a
  "Delegated rules" row → spec 24.
- `21-container-build-flow.md`: update the Stage-3 rows to 3-2/3-3/3-4
  (mise languages)/3-5 (cargo-binstall bootstrap)/3-6 (cargo tools via
  `cargo binstall`); add acceptance criteria: "`cargo binstall -V` and
  `which topgrade` succeed after `make up`; an empty `layer_3/cargo.txt`
  does not break the build; `make down && make up` preserves cargo
  binaries."
...
- Manual / build verification:
  ...
  - `programs/generate_deps` tests green (`make test-deps` or
    `pytest programs/generate_deps/tests/` — confirm the exact target in
    the Makefile during implementation; **open question Q3**).

## §7 Risks / open questions

- **Q1 — `cargo-binstall` install method.** Confirm the exact release
  asset name (`cargo-binstall-x86_64-unknown-linux-musl.tgz`?) and that
  `tar -xz` into `$CARGO_HOME/bin` is the supported path, vs. the
  `https://cargo-binstall.app` installer script. Prefer the tarball for
  reproducibility. Resolve during implementation by checking
  https://github.com/cargo-bins/cargo-binstall/releases.
- **Q2 — `cargo binstall` non-interactive flag.** Confirm whether
  `--no-confirm` (and/or `--no-symlink`, `--secure` / `--disable-strategies`)
  is the right combination for a non-interactive build. `cargo binstall`
  may refuse crates without provenance unless `--no-confirm` /
  `--allow-unstable` etc. are passed. Decide the minimal safe flag set
  during implementation.
- **Q3 — Test target.** Confirm the Makefile target / invocation for the
  generator tests (e.g. `make test-deps`, or `pytest` directly). Check
  `03-makefile.md` and the Makefile during implementation.
...
- **Risk — `cargo binstall` availability for `topgrade`.** `topgrade`
  ships prebuilt binaries on GitHub releases (cargo-binstall
  compatible), so Layer 3-6 should succeed. If a build-time cargo entry
  ever lacks a prebuilt, `cargo binstall` errors (does not auto-fall
  back to source). The Rust packages rule (spec 24) requires build-time
  `layer = 3` cargo entries to have prebuilt binaries; source-only tools
  must go to `layer = 6`. Documented in spec 24 §3.
