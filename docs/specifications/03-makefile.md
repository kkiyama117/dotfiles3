...
| `build` | container | build the container image for the current host uid/gid via rootless Podman. Requires `$(USERNAME)`. Tags the image as `$(IMAGE)`. Build context (`container/`) ships `bind/layer_1_files/makepkg.conf` and `pacman_mirrorlist` alongside the Containerfile. |
...
| `test-zsh` | dotfile test | active (see [`08-automations.md`](08-automations.md)) | runs `zsh container/tests/zsh/zoxide_zi_test.zsh`. Exit 0. Does not touch the network or the container. |
