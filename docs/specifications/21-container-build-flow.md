...
| `toolchain` (`FROM build-prepass`) | 3-8 | `ARG HERDR_VERSION`/`HERDR_SHA256`; `curl` the pinned v0.7.3 `herdr-linux-x86_64`; `sha256sum -c` against the hardcoded SHA256; `install -D -m 0755` to `~/.local/bin/herdr`; `herdr --version` | Install `herdr` (terminal workspace manager for AI coding agents) as a `custom` Layer 3 prebuilt binary (I-HERDR1). Declared `manager = "custom"` in `packages.toml` (doc-only; not in any `layer_<N>/<manager>.txt`). No cache mount (one-shot download, same as cargo-binstall 3-6). | `/tmp/build-home/.zshenv`, `ARG HERDR_VERSION`/`HERDR_SHA256` |
