...
| `base` | 1-2 | `COPY bind/layer_1_files/pacman_mirrorlist /etc/pacman.d/mirrorlist` then `RUN pacman -Sy ... zsh` (as root) | Seed a Japan-optimized pacman-mirrors custom mirrorlist from the build context, then install the user's login shell so `make exec` works. Cache is purged after install. | `bind/layer_1_files/pacman_mirrorlist` |
