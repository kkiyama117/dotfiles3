...
| `toolchain` (`FROM build-prepass`) | 3 | Copy rendered Cargo config (3-2), `rustup-init` (3-3), mise-managed languages and tools including `herdr` (3-4), pi CLI npm install (3-5), `cargo-binstall` (3-6), cargo tools (3-7); cache mounts on `$CARGO_HOME/{registry,git}` (4-1/4-2 only; binstall uses the crates.io HTTP API) | Install the rendered Cargo config, then rustup, mise-managed languages and tools (including `aqua:ogulcancelik/herdr`), the pi coding agent CLI, and cargo binaries under XDG-compliant paths. | `/tmp/build-home/.zshenv`, `/tmp/build-home/.local/share/cargo/config.toml`, `dependencies/layer_3/cargo.txt` |
