...
| `toolchain` (`FROM build-prepass`) | 3 | Copy rendered Cargo config (3-2), `rustup-init` (3-3), mise-managed languages and tools including `pi` and `herdr` (3-4), `cargo-binstall` (3-6), cargo tools (3-7); cache mounts on `$CARGO_HOME/{registry,git}` (4-1/4-2 only; binstall uses the crates.io HTTP API) | Install the rendered Cargo config, then rustup, mise-managed languages and tools (including `aqua:earendil-works/pi` and `aqua:ogulcancelik/herdr`), and cargo binaries under XDG-compliant paths. | `/tmp/build-home/.zshenv`, `/tmp/build-home/.local/share/cargo/config.toml`, `dependencies/layer_3/cargo.txt` |
...
- The `toolchain` stage copies the rendered Cargo config from
  ...
  `$CARGO_HOME/` before rustup, mise-managed tools, cargo-binstall,
...
25. After `make up`, `podman exec <container> zsh -ic 'herdr --version'`
    exits 0; `podman exec <container> zsh -ic 'which herdr'` resolves under
    `$MISE_DATA_DIR/shims` (mise shim, not `~/.local/bin/herdr`). `herdr` is
    installed via Layer 3-4 `mise install --yes` alongside other mise-managed
    tools (I-HERDR1). Config at `~/.config/herdr/` is rendered by runtime
    `chezmoi apply` with update checks disabled (I-HERDR3). **Rollout:** after
    `make build` (Layer 1-7), existing deployments with a pre-migration
    `dotfiles_mise` volume must run: `make down`, then
    `podman volume rm dotfiles_mise`, then `make up`. Doing this before the
    first `make up` after the migration seeds the named volume with the aqua
