...
- Copy host `~/.config/pacman/makepkg.conf` to bind dir
- Allowlist in `container/.gitignore`
- Add `COPY bind/layer_1_files/makepkg.conf /etc/makepkg.conf` in Layer 1-2
...
Update specs 01, 03, 20, 21, host_config_list.

## Phase 3 — Test

Add `test_makepkg_conf_baked_into_layer_1_2` to `test_entrypoint.py`.

## Phase 4 — Build + smoke

`make build`, `make up`, verify `PKGEXT` and `paru --version`.

## Phase 5 — Review

A + C + E per spec 09 (Containerfile change).
